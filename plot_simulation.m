...
[state_evo, ctrl_evo, PI_evo, PI_csi_evo, wr_evo, tilt_evo, tilt_des_evo] = system_simulation(delta_p, ref_traj, N, dt, true);
...
if(plot_state)
    ...
    xlabel('[s]')
    ylabel('position [m]')
    axis([0 Tf_sim min(min(ref_traj(1:3,:)))-0.5 max(max(ref_traj(1:3,:)))+0.5])
    %Vel
    subplot(3,2,3)
